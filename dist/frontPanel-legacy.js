...
fp.lungModels = ["SimpleLung", "SptLung", "SygLung", "RLung"];
...
fp.paramTable = function (object, paramSet, container, label) {
	if (typeof object[paramSet] != "undefined") {
		...
		for (var id in object[paramSet]) {
			...
			var td = $("<td></td>").attr("title", fp.translate1(id, "long")).html(abrev + " :").appendTo(tr);

			var td = $("<td class=\"data\"></td>");
			if (param.calculated == true) {
				var value = Math.round(10 * fp.ventilator[id]) / 10;
				var dataSpan = $("<span></span>").attr("id", "data" + id).html(value).appendTo(td);
			} else {
				var input = $("<input></input>").attr("id", 'input' + id).attr("value", object[id]).attr("size", '3').attr("type", 'number').attr("step", param.step).attr("onFocus", 'this.select()').appendTo(td);
			}
			tr.append(td);
			var unitSpan = $("<td class='unit'>" + unit + "</td>").appendTo(tr);
...
fp.timeSeries2 = [{
	id: "Pao",
	dataSets: [{ dataSet: "timeData", key: "Pao" }]
}, {
	id: "Flung",
	dataSets: [{ dataSet: "timeData", key: "Flung" }]
}, {
	id: "Palv",
	dataSets: [{ dataSet: "timeData", key: "Palv" }]
}, {
	id: "PCO₂",
	dataSets: [{ dataSet: "timeData", key: "PCO2" }]
}];
...
fp.initDyGraph = function () {
	...
	if (fp.graphics.length > 1) {
		fp.sync = Dygraph.synchronize(fp.graphics, {
			selection: true,
			zoom: true,
			range: false
		});
...
fp.progressBar = function () {
	fp.pbTimer = setTimer(function () {}, fp.pbDelay);
};
...
fp.init = function () {
	if (typeof fp.ventilator == "undefined") {
		fp.ventilator = new sv[fp.ventModel]();
	}
	if (typeof fp.lung == "undefined") {
		fp.lung = new sv[fp.lungModel]();
	}
