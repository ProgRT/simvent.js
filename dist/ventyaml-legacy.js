...
var ventyaml = function () {
	function ventyaml(sourceNode) {
	...
	_createClass(ventyaml, [{
		...
		value: function updateVent() {
			...
			} else if (_typeof(this.json.Ventilateurs) == 'object') {
		...
		key: "createvent",
		value: function createvent(ventDesc) {
			if (typeof ventDesc == 'string') {
				if (ventDesc in sv) {
					var vent = new sv[ventDesc]();
				}
			} else if ((typeof ventDesc === "undefined" ? "undefined" : _typeof(ventDesc)) == 'object') {
				if (!("Mode" in ventDesc)) {
					var vent = new sv.PressureControler();
				} else if (ventDesc.Mode in sv) {
					var vent = new sv[ventDesc.Mode]();
				} else {
					console.log("Does not seems to be a valid vent type");
				}
		...
		key: "createlung",
		value: function createlung(lungDesc) {
			if (typeof lungDesc == 'string') {
				if (lungDesc in sv) {
					var lung = new sv[lungDesc]();
				}
			} else if ((typeof lungDesc === "undefined" ? "undefined" : _typeof(lungDesc)) == 'object') {
				if (!("Type" in lungDesc)) {
					var lung = new sv.SimpleLung();
				} else if (lungDesc.Type in sv) {
					var lung = new sv[lungDesc.Type]();
				} else {
					console.log("Does not seems to be a valid lung type");
				}
		...
		value: function updateGraph() {
			...
			if ("Courbes" in this.json) {
				...
				if ((typeof courbes === "undefined" ? "undefined" : _typeof(courbes)) == "object") {
					for (var i in courbes) {
						this.createWaveform(courbes[i]);
					}
				} else {
					console.log("ventyaml: Value for courbes must be a string list");
				}
			...
			if (!('Courbes' in this.json) && !('Courbe' in this.json) && !('Boucle' in this.json) && !('Boucles' in this.json)) {
				this.createWaveform("Flung");
			}
		}
	}, {
		key: "createWaveform",
		value: function createWaveform(courbe) {
			if (typeof courbe == "string") {
				...
				console.log("ventyaml: Value for courbes must be a string");
			}
		}
	}, {
		key: "createLoop",
		value: function createLoop(boucle) {
			if ((typeof boucle === "undefined" ? "undefined" : _typeof(boucle)) == "object" && 'x' in boucle && 'y' in boucle && boucle.x != null && boucle.y != null) {
				var fx = function fx(d) {
					return d[boucle["x"]];
				};
				...
				console.log("ventyaml: invalid loop description");
			}
		}
	}, {
		key: "toggleSource",
...
function ventyamlEverything(selector) {
	...
	for (var i in preS) {
		if (typeof preS[i].tagName != 'undefined') {
			//Why the hell must I filter this?
